                    ...
                    try {
                        ...
                        if (data.success){
                            ...
                            if (data.itemid) { //do not test data.aggregate or data.count otherwise it doesn't refresh value=0 or no value
                                ...
                                console.log(selectnode._node.id);
                                if (selectnode._node.id.indexOf('_stu') > 0) {
                                    var node = scope.Y.one('#ratingaggregate_stu' + itemid);
                                    node.set('innerHTML',data.aggregate);

                                    node = scope.Y.one('#ratingcount_stu' + itemid);
                                    if (data.count > 0) {
